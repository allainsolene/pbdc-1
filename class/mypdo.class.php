...
class mypdo extends PDO{
    ...
    private $PARAM_utilisateur='PPE_CAC'; // nom d'utilisateur pour se connecter
    private $PARAM_mot_passe='Valentine91'; // mot de passe de l'utilisateur pour se connecter
    private $PARAM_nom_bd='alae_cano_chopin_allain';
    ...
    if($tab['categ']=='famille'){	
		...
		$requete='select * from personnel where login="'.$tab['id'].'" and mdp_personnel=("'.$tab['mp'].'");';
		}
	}
    ...
    public function modif_famille_admin($tab)
    {
		...
		.'fonction2='.$this->connexion ->quote($tab['fonction2']) 
 		.' where identifiantF='.$this->connexion ->quote($tab['identifiant']) .';';
